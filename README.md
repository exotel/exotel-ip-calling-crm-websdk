...
This repo uses GitHub Actions for CI and optionally publishing to npm.

- **CI**: A build runs on PRs and pushes to `master`/`main`. Make this workflow a **required status check** in GitHub branch protection to block merges if build fails.
- **Manual publish to npm**: Run the GitHub Actions workflow **"Publish to npm (manual)"** and set `publish=true` (set `publish=false` to only build without publishing).

### Adding `NPM_TOKEN` for the publish workflow

Add the token in GitHub:

- Repo → **Settings** → **Secrets and variables** → **Actions** → **New repository secret**
- Name: `NPM_TOKEN`
- Value: your npm **granular access token** with permission to publish `@exotel-npm-dev/exotel-ip-calling-crm-websdk` (and any org policy requirements like 2FA/bypass-2FA as configured on npm).
